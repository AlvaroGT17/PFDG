...
def generar_documento_pdf(datos, firma_cliente_path, ruta_guardado=None):
    ...
    # Obtener fecha de generación en formato español
    ...
    # Ruta de guardado (por defecto, por mes en la carpeta de recepcionamientos)
    if not ruta_guardado:
        ruta_guardado = os.path.join(
            ruta_base, "documentos", "recepcionamientos", ahora.strftime("%Y-%m"))

    os.makedirs(ruta_guardado, exist_ok=True)
    ruta_pdf = os.path.join(ruta_guardado, nombre_archivo)

    # Renderizar y guardar como PDF con estilo
